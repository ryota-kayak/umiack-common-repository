...
*   **ティア1：Loader（読み込み指示役）**:
    *   ローダー本体（`top-loader-coffee.js`, `kayak-tours-loader.js` 等）は、専用の[設定ファイル](file:///Users/ryota/Documents/Business/My%20business/umiack/Web/common-repository/src/server/common/umiack-site-assets/js/.htaccess)で `Cache-Control: no-cache` を設定。
...
*   **利点**:
*   **ロゴ・アニメーションのベストプラクティス (Animation Best Practices)**:
    *   **transformの活用**: `top/left` ではなく `transform: translate()` を使用することで、ブラウザのリフローを避け、滑らかで高パフォーマンスなアニメーションを実現します。
    *   **基準点の正規化**: CSSで `top: 0; left: 0; right: 0; margin: auto;` に設定し、中央を基準点（0,0）として JS から `translate` で制御することで、レスポンシブな位置調整が容易になります。
    *   **境界値のリセット**: スクロールを一番上に戻した際、明示的に初期状態へ戻すガード節（`if (scrollPosition <= delay) { ... }`）を設けることで、動作の堅牢性を高めます。
